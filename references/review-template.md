...
| **Best technical** | Is custom code sitting where an official hook exists? Is middleware order correct given onion composition? Do the documentation-stated practices hold? Is the Python itself sound: boundaries typed, names carrying their kind and answering to English, the event loop never blocked, expensive resources opened once, stdlib before dependency? | `deepagents/extension-points.md`, `deepagents/middleware.md`, `deepagents/best-practices.md`, `python-practice.md` |
