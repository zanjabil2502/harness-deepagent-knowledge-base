...
HEAD = """# Glossary — looking up terms and symbols
...
def check_terms() -> list[str]:
    ...
    for term, _, canon in TERMS:
        ...
        if not f.exists():
            errs.append(f"{term!r}: canonical file {canon} does not exist")
        elif probe not in f.read_text(encoding="utf-8").lower():
            errs.append(f"{term!r}: {canon} no longer contains {probe!r}")
    return errs


def main() -> int:
    if not GRAPH.exists():
        print(f"FAIL: {GRAPH.relative_to(ROOT)} does not exist")
        return 1
    ...
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"OK: {OUT.relative_to(ROOT)} — {len(TERMS)} terms, "
