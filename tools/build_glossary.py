...
TERMS = [
    ...
    ("background consolidation", "Writing memory between conversations with a second agent rather than on the hot path; removes user-facing latency at the cost of a lag before a fact becomes reusable.", "concepts/memory.md"),
    ("episodic / procedural / semantic memory", "The three information types memory can hold: what happened, how to do a task, and plain facts. Each wants a different retrieval mechanism, and merging them serves all three badly.", "concepts/memory.md"),
    ("short-term vs long-term memory", "Short-term is the thread (messages plus scratch, held by the checkpointer, scoped to one `thread_id`); long-term is the store (namespaced, outliving every thread). A storage choice, not a property of the fact.", "concepts/memory.md"),
